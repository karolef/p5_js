...
// for saving the best bird in a JSON file
function keyPressed() {
  if (key === 'S') {
    let bird = allBirds[0];
    // let json = bird.brain.serialize();
    saveJSON(bird.brain, 'birdBrain.json');
    // serialize calls stringify (the below) for us
    // let json = JSON.stringify(bird);
    // save('bird.json',json);
    // console.log(json);
  }
}
// you can read brain.json with
// let birdBrain = NeuralNetwork.deserialize(brainJSON);
// bird = new Bird(birdBrain);
// because neural network accepts birds that already have brain
...
function setup() {
  ...
  populationCountSpan = select('#popCount');
...
function draw() {
  ...
  // Update DOM Elements
  populationCountSpan.html(popCount);
